...
bool PhysicsManager::CheckCircleCollisions(GameObject* go, GameObject* other)
{
	...
	double totalRadiusSquared = go->collider->radius + other->collider->radius;
...
bool PhysicsManager::CheckCircleCollisions(Vector2 position, double radius, GameObject* other)
{
	Vector2 centerCircle2 = other->getCenterPosition();

	//Calculate total radius squared
	double totalRadiusSquared = radius + other->collider->radius;
	totalRadiusSquared = totalRadiusSquared * totalRadiusSquared;
...
double PhysicsManager::DistanceSquared(double x1, double y1, double x2, double y2)
{
	double deltaX = x2 - x1;
	double deltaY = y2 - y1;
...
bool PhysicsManager::CheckRectCircleCollisions(GameObject* rect, Vector2 centerCircle, double radius)
{
	// temporary variables to set edges for testing
	double testX = 0;
	double testY = 0;
...
GameObject* PhysicsManager::CheckSphere(Vector2 position, double radius, GameObject* self = NULL)
{
	vector<GameObject*> go = GameObjectManager::GetInstance().GetGameObjects();
